...
async function postAPI(title, imageUrl, categoryId) {
    ...
    if (!response.ok) {
        throw new Error(`Failed to update API: ${response.statusText}`);
    }
...
function displayEditModal() {
    ...
    // modalButton.insertBefore(closeButton, modalEditHeader);


    // Ajout du footer et du boutons valider
    // const modalEditFooter = document.createElement('div');
    ...
    // validerButton.setAttribute('form', 'addPhotoForm');
    ...
    validerButton.addEventListener("click", function (event) {
        ...
        // addPhotoForm.submit();
        submitForm();
        console.log("check passage");
    });   
}
async function submitForm () {

        console.log("check passage2");
        ...
        try {
            const result = await postAPI(formData.get('title'), formData.get('imageUrl'), formData.get('categoryId'));
            console.log(result);
            console.log("check passage3");
        } catch (error) {
            console.error(error);
...
// //Fonction pour delete un works par son id
// async function deleteWorkById(workId) {
//     const url = `http://localhost:5678/api/works/${workId}`;
